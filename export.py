...
Usage:
  python export.py --ckpt checkpoints/small_best.pt
...
def main():
    ...
    if src.resolve() == dst.resolve():
        raise SystemExit("output path must differ from the source checkpoint")
    ...
    torch.save({k: ckpt[k] for k in KEEP if k in ckpt}, temporary)
